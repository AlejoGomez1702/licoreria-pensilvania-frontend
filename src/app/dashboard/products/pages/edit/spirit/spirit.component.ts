...
export class SpiritComponent implements OnInit 
{
  ...
  buildForm(firstBuild: boolean)
  {
    ...
    {
      const { category, unit, ...data } = this.spirit;
      this.form.reset({
  ...
  onSubmit()
  {
    ...
    this.spiritService.updateSpirit( this.spirit.id!, this.form.value ).subscribe(
      () => {
